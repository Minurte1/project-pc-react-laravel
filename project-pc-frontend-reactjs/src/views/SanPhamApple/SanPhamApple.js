...
class SanPhamApple extends React.Component {
  ...
  fetchData = async () => {
    try {
      const response = await fetch("http://localhost:8080/api/sanphamApple", {
        method: "GET",
        mode: "cors",
      });
      if (!response.ok) {
              ...
              <div className="fillter_TimKiem">
                ...
                <label className="container_InputRadio_TimKiem">
                  <input
                    ...
                    value="asc"
                    checked={
                      priceFilter === "0" && this.state.sortOrder === "asc"
                    } // Thêm "this."
                    onChange={() => {
                ...
                <label className="container_InputRadio_TimKiem">
                  <input
                    ...
                    value="desc"
                    checked={
                      priceFilter === "0" && this.state.sortOrder === "desc"
                    } // Thêm "this."
                    onChange={() => {
                      this.handlePriceFilterChange("0");
                      this.handleSortChange();
                ...
                sortedData.map((item, index) => (
                  <li key={index}>
                    <div className="product-top">
                      <a
                        onClick={() => this.handleViewSanPham(item)}
